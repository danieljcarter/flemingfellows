...
In this session, we will be using a simulated dataset that is based on a real dataset. We are going to read in this dataset, then we're going to tidy it up. 9 out of 10 statisticians will agree that data cleaning and tidying is their least favourite part of their job. Nevertheless, effort spent making a tidy dataset (or datasets!) will pay off with substantially less pain at the analysis stage.
...
This dataset is a simulated dataset based on a real dataset collected as part of the DREAMS study, which investigates a combination HIV prevention intervention in a cohort of adolescent girls and young women in Kenya, Zimbabwe, and South Africa. It should be stressed that this is not real data and none of the combinations of data in this dataset actually belong to a real individual.
...
```{r}
table1 %>% mutate(incidencera=cases/population) %>% 
  ggplot(aes(x = year, y = incidencera, color = country)) + geom_point() + geom_line()
...
We use pivot_longer() to say that the two columns 1999 and 2000 should be put into a two new columns: a new column of names (called year) and a new column of values (called cases). Try this with table 4a now. Make sure you understand how the two datasets are related. You might also see this called gather() in older version of R.
...
> Exercise: Tidy table4b in the same way, using relevant column names for names_to and values_to.
...
Pivoting wider is the opposite of pivoting longer and you use it when an observation is spread across multiple rows, as is the case in table2. Here the cases and the population are spread across multiple rows. To used pivot_wider we need to specify a column with names and a column with values. The names column provides the new column names and the count column provides the values for this new column. You might also see this as spread() in older R versions. See the code below:
...
dreams <- dreams %>% 
  mutate(age.group = 
            as.factor(
                case_when(age < 15 & gender == "M" ~ "Young children who are male",
                          age = 16 ~ "16 Years"
                          age = 17 ~ "17 Years",
                          age > 17 ~ "18-22 Years")))
#---
...
> Summative Exercise Pt 2: For each country, year, and sex, compute the total number of cases of TB. Make an informative visualisation of this data for a country of interest to you. Here is an example for Brazil. Confirm you know what each step does.

```{r}
who %>%
  gather(key, value, new_sp_m014:newrel_f65, na.rm = TRUE) %>% # turns the dataset from wide to long
  mutate(key = stringr::str_replace(key, "newrel", "new_rel")) %>% # replaces all instances of newrel with new_rel
  separate(key, c("new", "var", "sexage")) %>% # separates out the different strings
  dplyr::select(-new, -iso2, -iso3) %>% # drops new case columns and redundant country info
  separate(sexage, c("sex", "age"), sep = 1) %>% # separate sex and age column
